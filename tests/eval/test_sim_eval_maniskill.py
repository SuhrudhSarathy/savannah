...
periodically during training) on the ManiSkill ColorMatching task. Not testing
model quality — only that the rollout loop runs to completion without crashing.
"""
...
from savannah.tasks.maniskill import ManiSkillColorMatchingTask
...
# Cap episode length so the smoke test stays fast — ColorMatching-v1 is
# registered with max_episode_steps=500. We only care that the loop doesn't crash.
...
def test_sim_eval_loop_does_not_crash():
    ...
    config = DataSetConfig(
        repo_id="suhrudhsarathy/color-pick-and-place",  # unused — get_env() doesn't touch the dataset
        fps=20,
        cameras=["front_cam"],
        obs_horizon=1,
        action_horizon=ACTION_HORIZON,
    )
    task = ManiSkillColorMatchingTask(config=config, device=device)
