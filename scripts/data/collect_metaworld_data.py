...
def main() -> None:
    ...
    if args.push_only:
        ...
        print(f"Pushing existing local dataset {args.repo_id} to Hub ...")
    ...
    try:
        while successes < args.num_episodes and attempts < max_attempts:
            ...
            print(
                f"Attempt {attempts:>4d}: {status} ({len(frames):>3d} steps) | "
                f"saved {successes}/{args.num_episodes}"
            )
    ...
    print(
        f"\nDone: {successes}/{args.num_episodes} successful episodes in {attempts} attempts."
    )
